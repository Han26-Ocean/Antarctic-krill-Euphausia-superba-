...
def multi_cnn_3d_lstm_model(shape_1, shape_2):
    inputs_so = tf.keras.Input(shape_2)
    inputs_sla = tf.keras.Input(shape_2)
    inputs_chl = tf.keras.Input(shape_1)
    inputs_o2 = tf.keras.Input(shape_2)
    ...
    so_1, sla_1, chl_1, o2_1, sst_1, u_v_value_1 = \
        cnn_3d_lstm(inputs_so), cnn_3d_lstm(inputs_sla), cnn_3d_lstm(inputs_chl), \
        cnn_3d_lstm(inputs_o2), cnn_3d_lstm(inputs_sst), cnn_3d_lstm(inputs_u_v_value)
...
def cnn_3d_lstm(x):
    ...
    x = tf.keras.layers.ConvLSTM2D(filters=64, kernel_size=2, activation='relu')(x)

    x = tf.keras.layers.Conv2D(filters=64, kernel_size=1, activation='relu')(x)
